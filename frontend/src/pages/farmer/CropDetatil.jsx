...
const getActivityStyle = (type) => {
  switch (type) {
    ...
    case "fertilization":
      return { icon: Leaf, color: "text-green-500 bg-green-50" };
    case "pesticide":
      return { icon: Shield, color: "text-purple-500 bg-purple-50" };
    default:
      return { icon: Activity, color: "text-gray-500 bg-gray-50" };
  }
};
import HarvestPanel from "../../components/farmer/HarvestPanel";
...
const CropDetail = () => {
  ...
  //function to handle fetching harvest
  const fetchHarvest = async () => {
    const h = await getHarvestsByCrop(crop.id);
    setHarvests(h);
  };
  ...
  const daysUntilHarvest = crop?.harvestDate
    ? Math.max(
        0,
        Math.ceil(
          (new Date(crop.harvestDate) - new Date()) / (1000 * 60 * 60 * 24),
        ),
      )
    : null;
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="bg-amber-500 text-white px-4 py-2.5 rounded-2xl shadow-lg hover:bg-amber-600 transition-all flex items-center gap-2 font-black text-sm"
            >
              <TrendingUp size={16} /> Log Harvest
            </button>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-8 space-y-8">
        ...
        <div className="relative h-52 md:h-64 rounded-[2rem] overflow-hidden shadow-xl">
          <img
            src={
              crop.img ||
              "https://images.unsplash.com/photo-1500076656116-558758c991c1?q=80&w=600"
            }
            alt={crop.name}
          ...
          <span
            className={`absolute top-4 right-4 px-4 py-1.5 rounded-full text-xs font-black uppercase border ${
              crop.health === "Healthy"
          ...
          {[
            {
              ...
              value: daysUntilHarvest !== null ? `${daysUntilHarvest}d` : "—",
              icon: Clock,
            ...
            <div
              key={stat.label}
              className={`bg-white rounded-3xl p-5 border shadow-sm ${stat.border}`}
            >
              <div
                className={`w-10 h-10 rounded-2xl flex items-center justify-center mb-3 ${stat.color}`}
              >
                <stat.icon size={18} />
              </div>
        ...
        <div className="flex gap-2 bg-white rounded-2xl p-1.5 border border-gray-100 shadow-sm w-fit">
          {[
            ...
            >
              ...
              {tab === "harvests" && harvests.length > 0 && (
        ...
        {activeTab === "overview" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {crop.plantingDate && (
                    <div className="flex items-center gap-4 p-4 bg-green-50 rounded-2xl border border-green-100">
                      ...
                      <div>
                        <p className="text-xs font-black uppercase text-green-600 tracking-widest">
                          Planted
                        </p>
                  ...
                  {crop.harvestDate && (
                    <div className="flex items-center gap-4 p-4 bg-amber-50 rounded-2xl border border-amber-100">
                      ...
                      <div>
                        <p className="text-xs font-black uppercase text-amber-600 tracking-widest">
                          Expected Harvest
                        </p>
                        <p className="text-base font-black text-gray-800">
                          {new Date(crop.harvestDate).toLocaleDateString()}
              ...
              <div
                className={`rounded-[2rem] border p-6 ${crop.health === "Healthy" ? "bg-green-50 border-green-100" : "bg-red-50 border-red-100"}`}
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center ${crop.health === "Healthy" ? "bg-green-100" : "bg-red-100"}`}
                  >
                    {crop.health === "Healthy" ? (
                  ...
                  <div>
                    <p
                      className={`text-lg font-black ${crop.health === "Healthy" ? "text-green-700" : "text-red-700"}`}
                    >
                      {crop.health === "Healthy"
                        ? "Crop is Healthy"
                        : "Crop is At Risk"}
                    ...
                    <p
                      className={`text-sm font-medium mt-0.5 ${crop.health === "Healthy" ? "text-green-500" : "text-red-400"}`}
                    >
                      {crop.health === "Healthy"
                        ? "No issues detected. Keep monitoring regularly."
                        : "Disease reported. Take action immediately."}
                    </p>
                  </div>
                </div>
              </div>
            ...
            {loadingActivities ? (
              ...
              <div className="bg-white rounded-[2rem] p-14 text-center border border-gray-100">
                ...
                <p className="text-sm text-gray-300 font-medium mt-1">
                  Log your first activity for this crop
                </p>
              </div>
            ) : (
              <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden">
                <div className="divide-y divide-gray-50">
                  {activities.map((activity) => {
                    const { icon: Icon, color } = getActivityStyle(
                      activity.type,
                    );
                    return (
                      <div
                      ...
                      >
                        <div
                          className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 ${color}`}
                        >
                          <Icon size={18} />
                        </div>
            ...
            <>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {CROP_STATUSES.map((status) => (
                      <button
                        key={status}
                        onClick={() =>
                          setEditCropForm({ ...editCropForm, status })
                        }
              ...
              <div className="px-8 py-5 border-t border-gray-100">
                <button
                  ...
                  disabled={
                    editingCrop ||
                    !editCropForm.cropType ||
                    !editCropForm.plantingDate
                  }
        ...
        >
          ...
          {diseaseSuccess ? (
            <div className="flex-1 flex flex-col items-center justify-center gap-4 p-8">
              ...
              <p className="text-sm text-gray-400 font-medium text-center">
                Disease report logged successfully.
              </p>
            </div>
          ) : (
            <>
              <div className="flex-1 overflow-y-auto px-8 py-6 space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <ChevronRight
                      size={16}
                      className="absolute right-4 top-4 text-gray-300 rotate-90 pointer-events-none"
                    />
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {SEVERITY_LEVELS.map((level) => (
                      <button
                        key={level}
                        onClick={() =>
                          setDiseaseForm({ ...diseaseForm, severity: level })
                        }
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl">
            ...
            <p className="text-sm text-gray-400 font-medium text-center mb-6">
              ...
              <span className="font-black text-gray-700">{crop.name}</span> and
              all its data.
            </p>
